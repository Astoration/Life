...
void ZeroParticleSprite::Update(float _eTime) {
	...
	for (PARTICLE::iterator iter = particleList.begin(); iter != particleList.end(); iter++) {
		...
		if (ZeroInputMgr->GetClientPoint().x - 100 < this->Pos().x + (*iter)->Pos().x + (*iter)->Width() && ZeroInputMgr->GetClientPoint().x + 100 > this->Pos().x + (*iter)->Pos().x){
			if (ZeroInputMgr->GetClientPoint().y - 100 < this->Pos().y + (*iter)->Pos().y + (*iter)->Height() && ZeroInputMgr->GetClientPoint().y + 100 > this->Pos().y + (*iter)->Pos().y){
					SAFE_DELETE(*iter);
